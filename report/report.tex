...
\begin{document}
    ...
    Based on the previously mentioned (\Cref{sec:indirect_reciprocity}) relation between indirect reciprocity and relatedness, or image scoring and the greenbeard phenomenon, we proceed in our experiments with \greenbeards as a means for a static image scoring mechanism.
    ...
    As described in \Cref{sec:indirect_reciprocity}, image scoring is similar to relatedness/the greenbeard phenomenon which is why the concept of \textit{falsebeards} plays a significant role in our work.
...
%    {In the evaluation section you should describe the experiments that support the claims of the story of your report. For example, if you previously said ``We propose to combine model A with component B which leads to improved accuracy.'' you should back this up with experimental evidence here. Typically, this means that you should show the performance of model A with and without component B. May be your method warrants other kinds of such ablation studies, which you might include and describe here.}
    ...
    Lighter plotted lines are a subset of the simulations we have run to show the deviations.
    ...
    Green denotes \greenbeards which help out other seemingly altruistic agents (aka greenbeards and impostors).
    ...
    In \Cref{fig:exp2_sucker} we see that the amount of \suckers (aka altruists that help everyone) first increases a bit and then decreases steadily, while the population in general increases.
    ...
    The cowards on the other hand, while having the same birth-rate, experience lower die-off-rates due to running away from a faulty resource instead of helping.
    ...
    In \Cref{fig:exp2_sucker_rel} we see how these different types stack against each other when compared in relative terms.
    ...
    Looking at \Cref{fig:exp4_impostor} the first thing that can be noticed is that the population increases faster than in the \coward-\sucker only case (\Cref{fig:exp2_sucker}) which saturates around day 2.500.
    Converging at around day 1.500, this experiment shows closer tendencies to the \coward-\greenbeard scenario (\Cref{fig:exp3_greenbeard}).

    Further similarities to experiment 2 (\Cref{fig:exp2_sucker}) can be made out in regard to the progression of the population types.
    ...
    This adds to the belief that such an \impostor genome offsets the benefits of \greenbeards holding together, supporting our first hypothesis (see \Cref{sec:hypothesis}) that misinformation has a major impact on the effectiveness of cooperation.
    ...
    \begin{figure*}
        ...
        \begin{subfigure}[b]{0.475\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/exp4_impostor_rel_10}
            \label{fig:10_imp}
    ...
    In this case, greenbeard altruism showed its effectiveness again, similar to experiment 3 (\Cref{fig:exp3_greenbeard_rel}), where \cowards die out again in addition to \suckers.
