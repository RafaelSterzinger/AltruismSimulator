...
\begin{document}
    ...
    One of the first more extensive studies on information and its influence on a species' fitness was explored by \citeauthor{wallace_misinformation_1973} \cite{wallace_misinformation_1973} where he built upon the claim by \citeauthor{fisherGeneticalTheoryNatural1931} \cite{fisherGeneticalTheoryNatural1930} and proved its correctness, that is, if misinformation is spread it should highly deviate from the truth.
    ...
    As describe in \Cref{sec:indirect_reciprocity}, image scoring is similar to relatedness/the green beard phenomenon which is why the concept of false beards plays a rather significant role for our research.
    Described in multiple papers \cite{SelfishGeneRichard,roberts_kin_2019,west_altruism_2010}, cheating false beards play a significant role regarding cooperative behaviour.
    Here, false beards describe the act where an entity disguises himself to be a bearer of a green beard allele, but is not.
    ...
    For this reason, \citeauthor{SelfishGeneRichard} \cite{SelfishGeneRichard} argues that the green beard mechanism is of rare existence, hinting that cooperative behaviour based on the green beard phenomenon is rather unstable if misinformation about the true one's true strategy is spreaded.
    However, there are certain exceptions occurring in nature such as the discovery by \citeauthor{keller_selfish_1998} \cite{keller_selfish_1998} of a spiteful green beard allele in the red imported fire ant species.
    ...
    A more modern and very similar work to ours is the paper published by \citeauthor{szamado_deception_2016} where they specifically look at how gossip mechanisms in indirect reciprocity via image scoring influences cooperation.
    The concentrated on the same idea as our where they posed the question of how random errors during gossip or even deceptive communication give rise to the collapse of cooperation.
    ...
    Resulting from their experiments, they empirically showed that allowing to spread deceptive information turns the image scoring based reputation worthless and thus cooperation rarer or non-existing.
    Additionally, as we expect equal outcomes of our experiments, their findings act as a sanity check for our results.

    \cite{whitaker_indirect_2018}
    \cite{bhui_how_2019}
    ...
    This manifests itself in the way the non-chosen agent acts when in a faulty resource situation, where e.g. it could choose \todo{to share its resource with the agent who obtained a faulty one with the potential to end up dying itself instead.} to help the agent-to-die and potentially end up dying itself instead.
    ...
    Here an agent stands for a contractor, a resource for a job, with a faulty resource corresponding to a \todo{difficult}hard job. where only one of the contractors gets paid.
    ...
    \todo{In altruisitc scenarios, by} By taking on workload of the hard job from the other contractor who was bound to be fired, one is able to help the other survive with the risk of being fired oneself.
    ...
    Every night the surviving agents reproduce by themselves (aka \todo{asexual} no partner needed) leading to 1 or 2 offspring with a probability of \VProbKids.
    ...
    We build up on our previous experiment but change the outcome when agents come across a faulty resource.
    ...
    \todo{Building up on the previous experiment we introduce a fairer notion, where \todo{\suckers obtain a greenbeard gene which allows them to differentiate between their own kind and suckers}\greenbeards only help out other \greenbeards.}
...
%
    ...
    Green denotes \greenbeards which help out other seemingly altruistic agents (aka \todo{suckers}, greenbeards and impostors).
    ...
    \todo{This acts as an insurance of the quality of further expirements.}
...
%            \todo{leave this figure in as it shows that the total population varies a lot which cannot be seen as easily in plot 3}
...
%    \todo{play around with settings so that they do die out and the following experiment makes more sense}
    ...
    While there are some runs where the \suckers were able to gain traction and constitute the majority at around day 1000, by day 5000 the population consists of \todo{i think we should be more precise aka 80\% \cowards} pretty much \cowards only.
